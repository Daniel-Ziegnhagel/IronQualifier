...
class Stopwatch {
    ...
    function initialize() {
        ...
        logger = Log.getLogger("Stopwatch");
    }
    ...
    function startOrStop() {
        ...
        if (running) {
            ...
            if (masterTimer != null) {
                ...
                logger.debug("Stopwatch added as observer to MasterTimer");
            } else {
                System.println("Error: masterTimer is null in startOrStop");
                logger.error("masterTimer is null in startOrStop");
            }
        } else {
            System.println("Stopwatch stopped");
            logger.debug("Stopwatch stopped");
            if (masterTimer != null) {
                masterTimer.removeObserver(self);  // Beobachter entfernen
                logger.debug("Stopwatch removed as observer from MasterTimer");
            } else {
                System.println("Error: masterTimer is null in startOrStop");
                logger.error("masterTimer is null in startOrStop");
            }
    ...
    function stopTimer() {
        ...
        if (running) {
            ...
            if (masterTimer != null) {
                ...
                logger.debug("Stopwatch manually stopped");
            } else {
                System.println("Error: masterTimer is null in stopTimer");
                logger.error("masterTimer is null in stopTimer");
            }
        }
    }
    ...
    function updateOnMasterTimer() {
        ...
        if (running) {
            ...
            logger.debug("Stopwatch time updated: " + stopwatchTime.toString());
